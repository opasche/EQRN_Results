...
## =============================== PARAMETERS ===============================
save_path <- "Results/ts/EQRN_comp_pen_nopen_comprnn/"
...
for (Q_pred_levels in list(list(qs=quant_pred_plot, nm="dq"),
  ...
  err_quant_plt_np <- plot_rmse_quantile_ts(fit_eqrn_np, fit_grf, fit_gbex, fit_egam, y_train, X_test, y_test, Q_pred_levels$qs, pred_interm,
                                            interm_lvl, intermediate_quantiles, true_q_test_plot, factorize=TRUE, test_data="other",
                                            pred_interm_c=pred_interm_c, bias_variance=TRUE, crop_Rbv=c(0,0,0))
  err_quant_comp_plt <- plot_rmse_quantile_comp_ts(fit_eqrn_p, fit_eqrn_np, fit_grf, fit_exqar, fit_gbex, fit_egam, y_train, X_test, y_test,
                                                   Q_pred_levels$qs, pred_interm, interm_lvl, intermediate_quantiles, true_q_test_plot,
                                                   factorize=TRUE, test_data="other", pred_interm_c=pred_interm_c,
                                                   names_EQRN=c("EQRN best","EQRN unpen."), bias_variance=TRUE, crop_Rbv=c(0,0,0))
  plot(err_quant_plt_p$rmse_plot)
  plot(err_quant_plt_np$rmse_plot)
  plot(err_quant_comp_plt$rmse_plot)
  plot(err_quant_comp_plt$Rbv_plot)
  if(save_plots){
