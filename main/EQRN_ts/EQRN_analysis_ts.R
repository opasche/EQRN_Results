...
param_file_names <- c("bestval_comprnn.R",
                      "bestval_nopen_comprnn.R",
                      "bestval_compgrf.R",
                      "bestval_nopen_compgrf.R",
                      "bestval_comporacle.R",
                      "bestval_nopen_comporacle.R")
...
fe_out <- foreach(param_file=param_files, .errorhandling="stop", .combine=rbind) %fun% {
  source(param_file)
  ...
  plot(train_plot)
  if(save_plots){
    save_myplot(plt=train_plot, plt_nm=paste0(params_string, "_training.pdf"),
  ...
  for (Q_pred_levels in list(list(qs=quant_pred_plot, nm="dq"),
    ...
    rmse_quant_plt_np <- plot_rmse_quantile_ts(fit_eqrn, fit_grf, fit_gbex, fit_egam, y_train, X_test, y_test, Q_pred_levels$qs, pred_interm,
                                               ...
                                               pred_interm_c=pred_interm_c, bias_variance=TRUE)
    plot(err_quant_plt)
    plot(rmse_quant_plt_np$rmse_plot)
    if(save_plots){
      ...
      save_myplot(plt=rmse_quant_plt_np$rmse_plot, plt_nm=paste0(save_path, params_string, "_rmsequant_",Q_pred_levels$nm,".pdf"),
                  width = 75, height = 75, cairo=FALSE)
      save_myplot(plt=rmse_quant_plt_np$Rbv_plot, plt_nm=paste0(save_path, params_string, "_Rbvquant_",Q_pred_levels$nm,".pdf"),
