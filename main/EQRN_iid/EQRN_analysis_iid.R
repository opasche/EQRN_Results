...
fe_out <- foreach(param_file=param_files, .errorhandling="remove", .combine=rbind) %fun% {
  ...
  for(i in 1:nb_quantiles_predict){
    if(effective_dim_model(model)<2){
      pred_plot <- plot_predictions_1D(pred_eqrn[,i], pred_grf_test[,i], pred_true[,i], X_test, quantiles_predict[i])
      plot(pred_plot)
      if(!is.null(save_path)){
        ggsave(paste0(params_string, "_q",quantiles_predict[i]*10000,"_predX.pdf"), plot=pred_plot, device="pdf",
  ...
  if(effective_dim_model(model)==2){
    ...
    if (test_data != "grid"){
      ...
      #High quantile prediction with GRF
      pred_grf_test_2 <- predict(fit_grf, newdata = X_test_2, quantiles = quantiles_predict)$predictions
      # UNCONDITIONAL predicted quantile(s) (Y quantile on X_train)
      pred_unc_2 <- predict_unconditional_quantiles(interm_lvl = interm_lvl, quantiles = quantiles_predict, Y = dat$Y, ntest = ntest_2)
      # SEMI-CONDITIONAL predicted quantiles
      pred_semicond_2 <- predict_GPD_semiconditional(Y=dat$Y, interm_lvl=interm_lvl, thresh_quantiles=intermediate_quantiles,
                                                     interm_quantiles_test=pred_interm_2, quantiles_predict=quantiles_predict)
      # GROUND-TRUTH (y_test_2)
      pred_true_2 <- generate_theoretical_quantiles(quantiles = quantiles_predict, X = X_test_2, model = model, distr = distr, df = df)
      # EQRN predictions on X_test_2
      pred_eqrn_2 <- EQRN_predict(fit_eqrn, X_test_2, quantiles_predict, pred_interm_2, interm_lvl)
      # gbex and gpdGAM predictions on X_test_2
      pred_gbex_2 <- gbex_predict(fit_gbex, X_test_2, to_predict=quantiles_predict, intermediate_quantiles=pred_interm_2, interm_lvl=interm_lvl)
      pred_egam_2 <- predict_gpd_gam(fit_egam, X_test_2, to_predict=quantiles_predict, intermediate_quantiles=pred_interm_2, interm_lvl=interm_lvl)
    } else {
      X_test_2 <- X_test
    ...
    for(i in 1:nb_quantiles_predict){
      pred2D <- plot_predictions_2D(pred_eqrn_2[,i], pred_gbex_2[,i], pred_grf_test_2[,i], pred_egam_2[,i],
                                    pred_semicond_2$predictions[,i], pred_true_2[,i], X_test_2, quantiles_predict[i])
      plot(pred2D)
      if(!is.null(save_path)){
        # ggsave(paste0(params_string, "_q",quantiles_predict[i]*10000,"_pred2D.pdf"), plot=pred2D, device="pdf",
        #        path=save_path, width=200, height=140, units="mm", dpi=300)
        save_myplot(plt=pred2D, plt_nm=paste0(save_path, params_string,"_q",quantiles_predict[i]*10000,"_pred2D.pdf"),
      ...
      pred2D_ex <- plot_predictions_2D(pred_eqrn_2[,i]-pred_interm_2, pred_gbex_2[,i]-pred_interm_2,
                                       ...
                                       X_test_2, quantiles_predict[i])
      plot(pred2D_ex)
      if(!is.null(save_path)){
        # ggsave(paste0(params_string, "_q",quantiles_predict[i]*10000,"_pred2D_ex.pdf"), plot=pred2D_ex, device="pdf",
        #        path=save_path, width=200, height=140, units="mm", dpi=300)
        save_myplot(plt=pred2D_ex, plt_nm=paste0(save_path, params_string,"_q",quantiles_predict[i]*10000,"_pred2D_ex.pdf"),
                    width = 40, height = 40, cairo=FALSE)
      }
    }
  }
